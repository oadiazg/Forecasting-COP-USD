...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        model_optim = optim.Adam(
            ...
            betas=(0.9, 0.98), eps=1e-08, weight_decay=0
        )
        return model_optim
    ...
    def train(self, setting):
        ...
        # Save the StandardScaler fitted on training data so predict_future.py can load it
        try:
            import joblib
            train_data_for_scaler, _ = self._get_data(flag='train')
            if hasattr(train_data_for_scaler, 'scaler'):
                joblib.dump(train_data_for_scaler.scaler, os.path.join(path, 'scaler.pkl'))
                print('Scaler saved to:', os.path.join(path, 'scaler.pkl'))
